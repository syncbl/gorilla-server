...
ActiveRecord::Schema.define(version: 2020_12_10_054622) do
  ...
  create_table 'dependencies', id: false, force: :cascade do |t|
    ...
    t.index ['dependent_package_id'],
            name: 'index_dependencies_on_dependent_package_id'
    t.index %w[package_id dependent_package_id],
            name: 'index_dependencies_on_package_id_and_dependent_package_id',
            unique: true
    t.index ['package_id'], name: 'index_dependencies_on_package_id'
  end

  create_table 'endpoints',
               id: :uuid,
               default: -> { 'gen_random_uuid()' },
               force: :cascade do |t|
  ...
  create_table 'packages',
    ...
    t.index %w[user_id group_name],
            name: 'index_packages_on_user_id_and_group_name'
    t.index %w[user_id name],
            name: 'index_packages_on_user_id_and_name',
            unique: true
  ...
  create_table 'settings',
    ...
    t.index %w[endpoint_id package_id],
            name: 'index_settings_on_endpoint_id_and_package_id',
            unique: true
