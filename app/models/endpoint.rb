class Endpoint < ApplicationRecord
  ...
  def actualize!
    discarded_packages = []
    installed_packages = []
    settings.discarded.where(dependent: false).map { |s| s.package.all_dependencies(discarded_packages) }
    settings.kept.where(dependent: false).map { |s| s.package.all_dependencies(installed_packages) }
