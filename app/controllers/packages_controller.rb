class PackagesController < ApplicationController
  ...
  def search
    if params[:q].present? && params[:q].size >= MIN_NAME_LENGTH
      @pagy, @packages =
        pagy(
          Package::External.without_blocked.published.search_by_text(params[:q]),
          items: params[:items],
        )
    else
  ...
  def package_params
    params.require(:package).permit(:user_id, :name, :external_url, :replacement, :version, :package_type, :caption, :description)
  end
end
